...
contract('LunaSphere', function (accounts) {
  ...
  it('set the total supply upon deployment', function () {
    return LunaSphere.deployed().then(function(instance) {
      ...
      assert.equal(BigInt(totalSupply),3000000000000000000, 'sets the total supply to 1 bill')
      return tokenInstance.balanceOf(accounts[0])
    }).then(function(adminBalance) {
      assert.equal(BigInt(adminBalance), 3000000000000000000, 'sets the inital balance of the admin')
  ...
  it('Transfers Function Working', function() {
    return LunaSphere.deployed().then(function(instance) {
      tokenInstance = instance;
      return tokenInstance.transfer.call(accounts[1],BigInt(9999999999999999999));
    }).then(assert.fail).catch(function(error) {
      assert(error.message.indexOf('revert') >= 0, 'error must contain revert')
      return tokenInstance.transfer.call(accounts[1], 2500000000000000, {from: accounts[0]})
    }).then(function(success) {
      assert.equal(success, true, 'it returns true');
      return tokenInstance.transfer(accounts[1], 2500000000000000, { from: accounts[0]})
      ...
      assert.equal(receipt.logs[0].args.to, accounts[1], 'logs the account the tokens are tranferred to');
      assert.equal(receipt.logs[0].args.value, 2500000000000000, 'logs the transfer amount');
      return tokenInstance.balanceOf(accounts[1]);
    }).then(function(balance) {
      assert.equal(BigInt(balance), 2500000000000000, 'adds the amount receive')
      return tokenInstance.balanceOf(accounts[0]);
    }).then(function(balance) {
      assert.equal(BigInt(balance), 2997500000000000000, 'deducts the amount receive')
    })
  });
  ...
  it('approves tokens delegated transfer', function() {
    return LunaSphere.deployed().then(function(instance) {
      ...
      assert.equal(receipt.logs[0].args.value, 100, 'logs the authorized amount');
  ...
  it('handles delegated token transfer', function(){
    return LunaSphere.deployed().then(function(instance){
      ...
      assert(error.message.toString().indexOf('revert') >= 0, 'cannot transfer larger than allowed')
      return tokenInstance.transferFrom.call(fromAccount, toAccount, 10, {from:spendingAccount})
    }).then(function(success) {
      assert.equal(success, true)
      return tokenInstance.transferFrom(fromAccount, toAccount, 10, {from:spendingAccount})
    }).then(function(receipt) {
      assert.equal(receipt.logs.length, 2, 'triggers transfer event and adjust approve event');
      assert.equal(receipt.logs[0].event, 'Transfer', 'should be the transfer event')
      assert.equal(receipt.logs[0].args.from, fromAccount, 'logs the account the tokens are transferred from');
      assert.equal(receipt.logs[0].args.to, toAccount, 'logs the account the tokens are tranferred to');
  ...
  it('Mint function working', function() {
    return LunaSphere.deployed().then(function(instance) {
      ...
      assert.equal(receipt.logs.length, 1, 'triggers one event');
      assert.equal(receipt.logs[0].event, 'Transfer', 'Should be the Tranfer event');
      assert.equal(receipt.logs[0].args.to, accounts[0], 'logs the account the tokens are tranferred to');
